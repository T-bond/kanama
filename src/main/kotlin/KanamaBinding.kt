 ...
 *
 * `bootstrap.c` calls [init] exactly once, immediately after starting the
 * JVM, via the JNI invocation API (`FindClass` + `CallStaticVoidMethod`).
 * After this method returns, JNI is never used again — all subsequent
 * traffic between Godot and Kotlin goes through Panama (FFM).
 */
object KanamaBinding {
     ...
     * ```
     */
    private val godotVersionLayout: MemoryLayout = MemoryLayout.structLayout(
    ...
    private fun fetchGodotVersion(): GodotVersion {
        val getVersion = GodotFFI.lookup(
            "get_godot_version",
    ...
    /**
     * Overwrite `initialize` and `deinitialize` in the
     * `GDExtensionInitialization` struct with Panama upcall stubs that
     * land in Kotlin. bootstrap.c pre-filled the struct with C-side
     * defaults as a fallback; after this runs, Godot calls us through
     * Panama instead.
     *
    ...
    private fun installInitCallbacks(initPtr: Long) {
        ...
        val initializeHandle = lookup.findStatic(
            KanamaBinding::class.java, "initializeCallback", methodType,
        )
        val deinitializeHandle = lookup.findStatic(
            KanamaBinding::class.java, "deinitializeCallback", methodType,
        )

        val initializeStub = GodotFFI.linker.upcallStub(
            initializeHandle, lifecycleDescriptor, GodotFFI.arena,
        )
        val deinitializeStub = GodotFFI.linker.upcallStub(
            deinitializeHandle, lifecycleDescriptor, GodotFFI.arena,
        )
    ...
    fun initializeCallback(userdata: MemorySegment, level: Int) {
        ...
        if (level == INITIALIZATION_SCENE) {
            try {
                ...
                System.err.println("[kanama:kt] initialize callback failed: ${t::class.qualifiedName}: ${t.message}")
