 ...
 *
 * `get_variant_from_type_constructor(T)` returns a function pointer of
 * signature `(variant_out, typed_in)` that wraps a raw typed value
 * into a Variant. `get_variant_to_type_constructor(T)` is the reverse.
 *
 * We cache the downcall [MethodHandle] per [VariantType] so we only
 * pay the lookup + downcallHandle cost once per type per process.
 */
object VariantConverters {
    ...
    fun variantFromType(type: VariantType): MethodHandle = fromType.getOrPut(type.id) {
    ...
    fun variantToType(type: VariantType): MethodHandle = toType.getOrPut(type.id) {
        val addr = getToTypeCtor.invoke(type.id) as MemorySegment
        check(addr.address() != 0L) {
            "get_variant_to_type_constructor(${type.name}) returned NULL"
        }
        GodotFFI.linker.downcallHandle(addr, FunctionDescriptor.ofVoid(ADDRESS, ADDRESS))
    }
