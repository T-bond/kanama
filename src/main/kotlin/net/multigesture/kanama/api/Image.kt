...
class Image(handle: MemorySegment) : Resource(handle) {
     ...
     */
    fun saveExr(path: String, grayscale: Boolean = false): Long {
        return ObjectCalls.ptrcallWithStringAndBoolArgRetLong(saveExrBind, handle, path, grayscale)
    }
     ...
     */
    fun saveExrToBuffer(grayscale: Boolean = false): ByteArray {
        return ObjectCalls.ptrcallWithBoolArgRetByteArray(saveExrToBufferBind, handle, grayscale)
    }
    ...
    companion object {
        ...
        private const val SAVE_EXR_HASH = 3108122999L
        private val saveExrBind by lazy {
            ObjectCalls.getMethodBind("Image", "save_exr", SAVE_EXR_HASH)
        }

        private const val SAVE_EXR_TO_BUFFER_HASH = 3178917920L
