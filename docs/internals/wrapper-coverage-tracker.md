...
**COMPLETE (2026-06-22).** `@OverrideVirtual` lets a `@ScriptClass` override any engine virtual on its
attach-to class chain, on desktop/Android (JVM) and iOS (device-validated). The function name is the
virtual name (GDScript-style `fun _draw()`); the processor validates against a generated signature
table from `extension_api.json` and dispatches typed args + returns. Coverage page tracks virtuals as a
separate axis.
...
| 5.2 JVM implementation | opus | done | 2026-06-21: `@OverrideVirtual("_x")` lets a `@ScriptClass` override any engine virtual on its attach-to class chain. **New:** `@OverrideVirtual` annotation; `scripts/generate_virtual_signature_table.py` → `processor/src/main/resources/.../virtual-signatures.tsv` (1437 virtuals + inherits map, regenerated by `refresh_godot_api.sh`); `VirtualSignatureTable.kt` loader/resolver (walks inheritance); processor parse + **fail-closed validation** (unknown virtual / arity / void-vs-value mismatch all fail the build); `VirtualModel.returnType`; **return marshalling in `dispatchCall`** + `return_type` in the method list. Lifecycle annotations unchanged (returnType=null → identical output). VALIDATED: `VirtualOverrideProbe` (Control: `_get_minimum_size():Vector2` + `_has_point(Vector2):Bool`) → KSP emits correct method-list/has_method/dispatch (inspected), registrar compiles, `runtime_smoke` PASS (no regression). Live on-engine fire of a value-returning virtual = the 5.3 device demo. NOTE: arg-type *kind* validation deferred (only arity+voidness now); iOS still no-ops `@OverrideVirtual` (5.3) |
| 5.3a iOS void virtuals | opus | done | 2026-06-22: widened `IosScriptCodeEmitter.toIosMethod` past the fixed 12-lifecycle `when` — arbitrary **void** `@OverrideVirtual` virtuals dispatch through the proven generic call path. Device-validated together with 5.3b. |
| 5.3b iOS value-returning virtuals (callV return marshalling) | opus | done | 2026-06-22: **DEVICE-VALIDATED on iPhone 12.** Added the return-out primitive end to end: C `call_v` export + `kanama_ios_script_instance_call` callback gained `ret_tag`/`ret_buf` and build the engine return Variant via `kanama_ios_pt_arg_to_variant`; `KanamaIosRuntime` `callVReturning` + `encodeIosReturn` encode the method's return PT-tagged; `IosScriptCodeEmitter` emits `callVReturning` for value-returning methods/virtuals (Bool/Int/Float/Vector2/Vector2i). **Proof:** an iOS `Control` probe overriding `_get_minimum_size():Vector2` fired (`override dispatched -> (123,45)`) and `_has_point(Vector2):Bool` returned correctly through `self.call` (`at150=true at5=false` — value-return + Vector2 arg both marshalled). Matrix 54/78, 0 crashes. **API CHANGE (forced by KSP2):** `@OverrideVirtual` is now a **no-arg marker; the Kotlin function name IS the virtual name** (GDScript-style `fun _draw()`), because KSP2-over-Native does not expose function-annotation argument values (class-annotation args like `attachTo` do work). Supersedes the 5.1 explicit-`@OverrideVirtual("_x")` decision. String/Variant/Packed returns deferred. |
| 5.4 Coverage accounting for virtuals | sonnet | done | 2026-06-22: `ApiMethod` gains `is_virtual`; `api_wrapper_coverage.py` excludes engine virtuals from callable-method coverage (now **14957/15385 = 97.2%**, previously diluted to ~89% by the 1437 virtuals) and adds a dedicated **Virtual Methods** axis (1437, overridden via `@OverrideVirtual`, validated against the generated signature table). The generator report already labels virtuals as a "Virtual/internal" skip category (1437), so neither report counts them as wrapper gaps. `docs/reference/api-coverage.md` regenerated. **PHASE 5 COMPLETE.** |
...
- **Phase 4 breadth batches 1–12** — emitted iOS classes **27→222** (~3,800+ new wrapper methods). All
  ...
  **Batch 5 (2026-06-20): +14 — Button + 5 BaseButton/Button children (CheckBox/CheckButton/OptionButton/
  ...
  +279 wrapper methods; DEVICE-VALIDATED (regression-clean 54/78).**
