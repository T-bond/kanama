...
| 5.3a iOS void virtuals | opus | done (device-pending) | 2026-06-22: widened `IosScriptCodeEmitter.toIosMethod` past the fixed 12-lifecycle `when` — arbitrary **void** `@OverrideVirtual` virtuals (e.g. `_draw`, `_gui_input`, `_drop_data`) now emit as dispatchable iOS methods through the proven generic call path (same as a regular method); the engine calls them once they appear in the script method list. Value-returning virtuals still skip+warn (5.3b). Emit-only, no C/header change → compileKotlinIosArm64 clean (no regression; current iOS scripts use only lifecycle virtuals). DEVICE VALIDATION PENDING — needs an iOS script overriding an observable void virtual + an iPhone run. |
| 5.3b iOS value-returning virtuals (callV return marshalling) | opus | todo | the one new primitive: `kanama_ios_script_instance_call_v` (C, `kanama_ios_shim.c:5425` callback + `:59` export) has NO return slot — add a PT-tagged return-out (reuse `kanama_ios_pt_arg_to_variant` to build the engine `ret` Variant), have the generated iOS `callV`/`KanamaIosRuntime` encode the method's return PT-tagged. Touches the C shim + cinterop header → cinterop regen + DEVELOPER_DIR gradle build + device test. |
