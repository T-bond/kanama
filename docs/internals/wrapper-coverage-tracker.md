...
| 5.2 JVM implementation | opus | todo | **READY** — 5.1 decisions made (2026-06-21): `@OverrideVirtual` + lifecycle aliases; generated virtual-signature table from `extension_api.json` + validate fail-closed; curated first-cut subset (`_draw`/`_gui_input`/`_get_minimum_size`/`_get_configuration_warnings`/drag-drop trio). Start: build the signature table, add `@OverrideVirtual`, extend `VirtualModel` (+returnType/args), mint virtuals from the annotation, validate, add a demo overriding a non-lifecycle virtual |
