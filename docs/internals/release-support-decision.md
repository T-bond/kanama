...
| Android (second model, debug) | **Galaxy S10+** SM-G975U (physical, Adreno 640) | Android 12 | OpenGL Compatibility (demo default) | Nine-demo debug matrix green (2026-07-13) after the pre-Android-16 fixes (PanamaPort fork `SDK_INT_FULL_COMPAT`, kanama `Paths.get`). **Release-mode builds (minified or not) FAIL on this device** — task-42 forensics: `android:debuggable=false` triggers a PanamaPort A12 FFI crash into system libLLVM (tombstoned); not an R8 issue (a separate real R8 landmine was found and fixed en route, fork r8.4). Release floor recorded as Android 13+ | [exporting/android.md](../exporting/android.md) "Validated Android Versions", §7 B1 |
| Android (third model, debug) | **Moto g 5G 2023** (physical, Adreno 619) | Android 14 | OpenGL Compatibility (demo default) | Nine-demo debug matrix green (2026-07-13), PanamaPort resolved from the **published JitPack r8.3 artifact** (no local override). Heavy demo (third-person) needs an extended launch wait (`KANAMA_ANDROID_LAUNCH_WAIT=120`; first-run GL shader compile on a budget GPU) and runs with some choppiness — unprofiled; candidate task-12 workload (first step: A/B vs the upstream GDScript demo on the same device) | [exporting/android.md](../exporting/android.md) |
| Android (floor probe) | **Pixel 3 XL** (physical, Adreno 630) | Android 9 (factory image, API 28) | **Vulkan Forward Mobile** (`KANAMA_ANDROID_RENDERER=mobile`) | Dodge debug smoke green (2026-07-13; Vulkan 1.1.66 asserted). The factory 2018 Adreno **GL** driver segfaults linking Godot 4.7 compat shaders pre-Kanama — engine/vendor constraint, see exporting/android.md Current Boundaries | [exporting/android.md](../exporting/android.md) |
...
- **Android row** — replace `Emulator-validated (4.7 stable); Pixel 7 pending`
  ...
  fork `com.github.falcon4ever.PanamaPort:Core:0.1.3-kanama-r8.4`, not upstream.
...
- **R8/minification depends on the PanamaPort fork.** Minified release APKs are
  validated only with `com.github.falcon4ever.PanamaPort:Core:0.1.3-kanama-r8.4`.
...
|---|---|---|---|---|
| **B1** | Device matrix breadth | ≥ 2 physical device models per platform pass that platform's full repeatable gate on the current Godot baseline | ✅ **MET** — iPhone 12 (2026-06-25) + iPhone 15 Pro (2026-07-10) full ten-step gates (§2) | ✅ **MET (2026-07-13)** — **Moto g 5G 2023** (Adreno 619, Android 14) passes the **full gate**: nine-demo debug matrix + R8-minified Match3 release, PanamaPort resolved from the published JitPack r8.3 artifact — joining **Pixel 7** (Android 16, 2026-06-26/2026-07-11) as the second full-gate model. Additional breadth the promotion wording can cite: **Galaxy S10+** (Adreno 640, Android 12) passes the nine-demo debug matrix after the pre-Android-16 fixes (fork `SDK_INT_FULL_COMPAT` + kanama `Paths.get` `cb9ea40b`) but **fails release-mode builds on Android 12** (task-42 forensics complete: `debuggable=false` triggers a PanamaPort A12 FFI crash into system libLLVM — upstream-shaped, tombstoned, DEFERRED #10; a separate genuine R8 landmine — `filled-new-array [[B`, ART <13 interpreter crash — was found and fixed en route, fork r8.4); **Pixel 3 XL** (Adreno 630, factory Android 9 / API 28) passes the dodge debug smoke under Vulkan (`KANAMA_ANDROID_RENDERER=mobile`; its 2018 Adreno GL driver segfaults pre-Kanama — engine/vendor constraint, exporting/android.md). **Min-version statement (maintainer floor decision 2026-07-13, live in exporting/android.md "Validated Android Versions"):** debug validated down to Android 9; release builds require Android 13+, validated on 14 + 16 |
...
- **Android third-party dependency exposure.** The Android path depends on the
  ...
  (`com.github.falcon4ever.PanamaPort:Core:0.1.3-kanama-r8.4`) — documented in
...
**Assessment (2026-07-13): the promotion bar is fully green — B1–B4 MET on
both platforms.** The Moto g 5G (Android 14) full gate (debug + R8-minified,
on the published JitPack r8.3 artifact) closed the last criterion. Promotion
from Experimental to Supported is now a **wording decision** for the
maintainer, carrying the §7 packaging caveats, the min-version statement
(maintainer floor decision 2026-07-13: debug validated to Android 9; release
builds require Android 13+, validated on 14+16 — task 42 resolved with full
forensics, the A12 release-mode PanamaPort constraint is documented and
upstream-shaped), and the disclosed non-gating follow-ups (task 26
mobile multiplayer UI + the untested cross-platform join matrix). The
2026-07-13 device sessions also surfaced that the declared `minSdk = 26` had
never been validated below Android 16 before — the four-model matrix (Pixel
3 XL / S10+ / Moto g / Pixel 7 across Android 9/12/14/16) is the new
evidence base.
