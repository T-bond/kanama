...
/**
 * Canonical engine-virtual signatures, loaded from the generated
 * `virtual-signatures.tsv` resource (see
 * `scripts/generate_virtual_signature_table.py`, derived from
 * `extension_api.json`).
 *
 * Used by the processor to resolve and validate `@OverrideVirtual` declarations
 * against a script's attach-to class hierarchy. Resolution walks the class
 * inheritance chain so a script attached to e.g. `Sprite2D` can override
 * `CanvasItem._draw`.
 */
internal object VirtualSignatureTable {
    ...
    init {
        ...
        val stream = VirtualSignatureTable::class.java.getResourceAsStream(RESOURCE)
    ...
    /**
     * Resolve [virtualName] for a script attached to [attachTo], walking the
     * inheritance chain (attach-to class first, then ancestors). Returns the
     * canonical [Sig] or `null` if no class in the chain declares it.
     */
    fun resolve(attachTo: String, virtualName: String): Sig? {
        var cls: String? = attachTo
        val seen = HashSet<String>()
        while (cls != null && seen.add(cls)) {
            virtualsByClass[cls]?.get(virtualName)?.let { return it }
            cls = inherits[cls]
        }
        return null
    }
