...
| `docs/internals/ios-backend-handwritten.md` | `scripts/ios_handwritten_report.py` | Generated report; do not hand-edit. |
...
|---|---|---|
| `REFCOUNTED_REFERENCE_HASH == REFCOUNTED_UNREFERENCE_HASH` in `ScriptBridge.kt` | `reference()` and `unreference()` share the `bool()` signature, so their method hashes match by construction. | Verified legitimate in `docs/internals/architecture-review-2026-06.md` verdict/F5 observations. |
...
| Single-class iOS wrapper regeneration can drop methods returning other wrapper types | The generator emits returns only for classes in the active iOS wrapper set. | Regenerate with the full emitted-class union; see `docs/internals/ios-demo-port-tracker.md` "Generator Gotcha". |
