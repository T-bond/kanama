...
class HelloKanama(val godotObject: MemorySegment) {
    ...
    @RegisterProperty(hint = PropertyHint.RANGE, hintString = "0,100,1")
    var counter: Long = 0
    ...
    @OnReady
    fun ready() {
        System.err.println("[kanama:kt] HelloKanama._ready() obj=0x${godotObject.address().toString(16)}")
    }

    @OnEnterTree
    fun enterTree() {
        System.err.println("[kanama:kt] HelloKanama._enter_tree() obj=0x${godotObject.address().toString(16)}")
    }

    @OnExitTree
    fun exitTree() {
        System.err.println("[kanama:kt] HelloKanama._exit_tree() obj=0x${godotObject.address().toString(16)}")
    }
